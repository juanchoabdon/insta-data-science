...
#session.set_ignore_if_contains(['glutenfree', 'tasty'])
...
#session.like_by_tags(['#tech','#programming','#entrepreneurship','#nodejs'], amount=200)
...
#session.like_by_users(usernames=['freddiervega', 'cvandercito'], amount=100, random=True)
...
#.follow_user_following('friend2', amount=10, random=True, sleep_delay=60)
# For 50% of the 30 newly followed, move to their profile
# and randomly choose 5 pictures to be liked.
# Take into account the other set options like the comment rate
# and the filtering for inappropriate words or users
